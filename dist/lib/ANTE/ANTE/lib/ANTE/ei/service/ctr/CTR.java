...
/**
 * This class provides the Computational Trust and Reputation service. The CTR service receives information from Normative Environment service regarding the
 * activity of external agents and provide a list of  external agents that fulfill the provided requirements, and information about agents.

 * The computational trust and reputation service implements the following interaction protocol roles:
 * <ul>
 * <li>the CTR responder role
 * <li>the NormEnv subscription initiator role
 * <li>the DF subscription initiator role
 * </ul>
 * 
 * @author Sérgio
 */
public class CTR extends PlatformService{
...
//			addBehaviour(new SyncResetResp(this, getSynchronizer()));
	 ...
	 * The CTR subscribes the DF for updates on existing agents and their services (roles).
	 * 
	 * @author sergio
	 */
...
//					}
			      ...
			      for(int i = 0; i < eAgInfo.size(); i++)
			      {
			    	  ...
			    	  {   
					      ...
					      if (getGUI() != null) {
					    	  ...
					    	  ((CTRGui) gui).addValuesRow(mapMet, eAgInfo.get(i).getContexts().get(0).getSeller(),eAgentInfo.getNContracts(), eAgentInfo.getCTREval(false, null, "")); 
					      }					      
			    	  }
			      }
			    }
			    ...
			    if(((EIAgent) myAgent).isInSynchronizedExperiment()) {
				    // enviar informação final de execução do contrato para o synchronizer
	...
	/**
	 * Inner class to implement the Synchronization responder role.
	 * 
	 * @author hlc
	 */
	...
	/**
	 * Inner class to implement the CTR responder role.
	 * The CTR agent will receive requests on trust information of agents.
	 * 
	 * @author hlc
	 */
	private class CTRResp extends SimpleAchieveREResponder {
		...
		/**
		 * Creates a <code>CTRResp</code> instance.
		 * 
		 * @param agent The agent that adds the behaviour.
		 */
		CTRResp(Agent agent) {
		...
		protected ACLMessage prepareResponse(ACLMessage request) {
			...
			if(request_action instanceof SendInfo || request_action instanceof SendCTREval || 
					request_action instanceof SendTopAgents)
				// if requested action is known and can have any sender, agree
